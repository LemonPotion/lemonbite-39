...
const QuickOrder: React.FC = () => {
  ...
  if (savedOrders.length === 0 && !showSavePrompt) {
    return (
      <div className="bg-white/60 rounded-lg p-4 shadow-sm border border-[#F77A54]/20">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="text-xs px-3 py-1 bg-[#F77A54]/10 hover:bg-[#F77A54]/20 text-[#F77A54] rounded-full transition-colors flex items-center space-x-1"
          >
            <Bookmark size={14} />
            <span>Save Current</span>
          </button>
        </div>
        <p className="text-xs text-[#2E2E2E]/60 mt-2">Save your favorite orders for quick reordering!</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/60 rounded-lg p-4 shadow-sm border border-[#F77A54]/20">
      ...
      {showSavePrompt ? (
        <div className="bg-[#F2F0E3] p-3 rounded-md mb-2">
          <input
            type="text"
            placeholder="Order name (e.g. 'Family Dinner')"
            value={orderName}
            onChange={(e) => setOrderName(e.target.value)}
            className="w-full mb-2 px-3 py-2 rounded border border-[#F77A54]/20 text-sm focus:outline-none focus:ring-1 focus:ring-[#F77A54]/50"
          />
          <div className="flex space-x-2">
            <button 
              onClick={confirmSaveOrder}
              className="flex-1 text-xs px-3 py-2 bg-[#F77A54] text-white rounded-md transition-colors hover:bg-[#F77A54]/90"
            >
              Save Order
            </button>
            <button 
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {savedOrders.map((order, index) => (
            <div key={index} className="bg-[#F2F0E3] p-2 rounded-md flex items-center justify-between">
              <div className="flex items-center space-x-2">
              ...
              <div className="flex space-x-1">
                <button 
                  ...
                  className="text-xs px-2 py-1 bg-[#F77A54]/10 hover:bg-[#F77A54]/20 text-[#F77A54] rounded transition-colors"
                >
                  Load
                </button>
                <button 
                  onClick={() => deleteOrder(index)}
                  className="text-xs px-2 py-1 bg-gray-200 hover:bg-gray-300 text-[#2E2E2E] rounded transition-colors"
                >
                  ✕
                </button>
              </div>
