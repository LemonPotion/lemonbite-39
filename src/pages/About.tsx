...
import { Code, Database, Server, Terminal, Laptop, Layers, FileCode, Link as LinkIcon, ChartBar, Sparkles, GitFork, Github, Maximize2, Minus, X } from "lucide-react";
...
const About = () => {
  ...
  const techStacks = {
    backend: [
      { name: "ASP.NET Core", description: "Современный кроссплатформенный веб-фреймворк", icon: <Server className="text-accent" size={40} /> },
      { name: "Entity Framework Core", description: "Современная ORM для .NET", icon: <Database className="text-accent" size={40} /> },
      { name: ".NET 8", description: "Новейшая платформа .NET", icon: <Code className="text-accent" size={40} /> },
      { name: "PostgreSQL", description: "Продвинутая база данных с открытым исходным кодом", icon: <Database className="text-accent" size={40} /> },
      { name: "MediatR", description: "Реализация паттерна CQRS", icon: <Layers className="text-accent" size={40} /> },
      { name: "AutoMapper", description: "Решение для маппинга объектов", icon: <FileCode className="text-accent" size={40} /> },
    ],
    frontend: [
      { name: "React 18", description: "Современная UI библиотека", icon: <Laptop className="text-accent" size={40} /> },
      { name: "TypeScript", description: "Типизированный JavaScript", icon: <Terminal className="text-accent" size={40} /> },
      { name: "Tailwind CSS", description: "CSS фреймворк основанный на утилитах", icon: <FileCode className="text-accent" size={40} /> },
      { name: "React Query", description: "Библиотека для получения данных", icon: <LinkIcon className="text-accent" size={40} /> },
      { name: "Framer Motion", description: "Библиотека анимаций", icon: <ChartBar className="text-accent" size={40} /> },
      { name: "shadcn/ui", description: "Библиотека UI компонентов", icon: <Layers className="text-accent" size={40} /> },
    ]
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 py-10">
        <motion.div 
          ...
          className={`macOS-window border border-border rounded-xl overflow-hidden shadow-xl mx-auto ${isFullscreen ? 'fixed inset-0 z-40 m-0 rounded-none' : 'max-w-6xl'}`}
          style={{ 
        ...
        >
          {/* MacOS Window Header */}
          <div 
            className="macOS-window-header flex items-center px-4 py-2 border-b border-border"
            style={{ 
              backgroundColor: theme === 'dark' ? '#2A2A2A' : '#f0f0f0',
            }}
          >
            <div className="flex items-center space-x-2">
              <button className="w-3 h-3 rounded-full bg-red-500 flex items-center justify-center group">
                <X className="w-2 h-2 text-red-800 opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
              <button className="w-3 h-3 rounded-full bg-yellow-500 flex items-center justify-center group">
            ...
            <div className="flex-grow text-center">
              <h4 className="text-sm font-medium text-foreground/80">LemonBite - Презентация</h4>
            </div>
            <div className="w-16"></div> {/* Spacer to balance the window controls */}
          </div>

          {/* Mac OS Window Content Area (Scrollable) */}
          <div className="macOS-window-content overflow-y-auto h-[calc(100vh-120px)]">
            {/* Title Slide */}
          ...
          {/* MacOS Window Footer */}
          <div 
            className="macOS-window-footer border-t border-border px-4 py-2 flex items-center justify-between"
            style={{ backgroundColor: theme === 'dark' ? '#2A2A2A' : '#f0f0f0' }}
          >
            <div className="text-xs text-foreground/60">Slide 1 из 4</div>
            <div className="flex items-center space-x-2">
